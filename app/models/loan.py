...
class Loan(Base):
    ...
    holder_member_id: Mapped[Optional[int]] = mapped_column(ForeignKey("team_member.id"), default=None)
    holder_party_id: Mapped[Optional[int]] = mapped_column(ForeignKey("party.id"), default=None)
    ...
    holder_member: Mapped[Optional["TeamMember"]] = relationship(foreign_keys=[holder_member_id])
    holder_party: Mapped[Optional["Party"]] = relationship(foreign_keys=[holder_party_id])
    ...
    def holder_display(self) -> str:
        ...
        if self.holder_member is not None:
            return self.holder_member.full_name
        if self.holder_party is not None:
            return self.holder_party.name
        return "?"
